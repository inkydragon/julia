...
may trip up Julia users accustomed to MATLAB:
  ...
  * Julia does not automatically grow arrays in an assignment statement. Whereas in MATLAB `a(4) = 3.2`
    ...
    if this statement is the first use of the identifier `a`. Julia has [`push!`](@code-self-ref) and [`append!`](@code-self-ref),
    which grow `Vector`s much more efficiently than MATLAB's `a(end+1) = val`.
  * The imaginary unit `sqrt(-1)` is represented in Julia as [`im`](@code-self-ref), not `i` or `j` as in MATLAB.
  ...
  * Julia has true one-dimensional arrays. Column vectors are of size `N`, not `Nx1`. For example,
    [`rand(N)`](@code-self-ref) makes a 1-dimensional array.
  * In Julia, `[x,y,z]` will always construct a 3-element array containing `x`, `y` and `z`.
    - To concatenate in the first ("vertical") dimension use either [`vcat(x,y,z)`](@code-self-ref) or separate
      with semicolons (`[x; y; z]`).
    - To concatenate in the second ("horizontal") dimension use either [`hcat(x,y,z)`](@code-self-ref) or separate
      with spaces (`[x y z]`).
    - To construct block matrices (concatenating in the first two dimensions), use either [`hvcat`](@code-self-ref)
  ...
  * In Julia, `a:b` and `a:b:c` construct `AbstractRange` objects. To construct a full vector like in MATLAB,
    use [`collect(a:b)`](@code-self-ref). Generally, there is no need to call `collect` though. An `AbstractRange`
    ...
    and is also seen in functions such as [`range`](@code-self-ref), or with iterators such as `enumerate`, and
  ...
  * Functions in Julia return values from their last expression or the `return` keyword instead of
    listing the names of variables to return in the function definition (see [The return Keyword](@ref man-func-return)
    for details).
  * A Julia script may contain any number of functions, and all definitions will be externally visible
    ...
    directory.
  * In Julia, reductions such as [`sum`](@code-self-ref), [`prod`](@code-self-ref), and [`max`](@code-self-ref) are performed
    over every element of an array when called with a single argument, as in `sum(A)`, even if `A`
    has more than one dimension.
  * In Julia, parentheses must be used to call a function with zero arguments, like in [`rand()`](@code-self-ref).
  * Julia discourages the use of semicolons to end statements. The results of statements are not
    ...
    with semicolons. [`println`](@code-self-ref) or [`@printf`](@code-self-ref) can be used to print specific output.
  * In Julia, if `A` and `B` are arrays, logical comparison operations like `A == B` do not return
    ...
    [`<`](@code-self-ref), [`>`](@code-self-ref).
  * In Julia, the operators [`&`](@code-self-ref), [`|`](@code-self-ref), and [`⊻`](@ref xor) ([`xor`](@code-self-ref)) perform the
  ...
  * Julia's [`svd`](@code-self-ref) returns singular values as a vector instead of as a dense diagonal matrix.
  ...
  * Julia's `struct`s do not support dynamically adding fields at runtime, unlike MATLAB's `class`es.
    Instead, use a [`Dict`](@code-self-ref). Dict in Julia isn't ordered.
  ...
  * In MATLAB, an idiomatic way to remove unwanted values is to use logical indexing, like in the
    ...
    provides the higher order functions [`filter`](@code-self-ref) and [`filter!`](@code-self-ref), allowing users
    to write `filter(z->z>3, x)` and `filter!(z->z>3, x)` as alternatives to the corresponding transliterations
    `x[x.>3]` and `x = x[x.>3]`. Using [`filter!`](@code-self-ref) reduces the use of temporary arrays.
...
For users coming to Julia from R, these are some noteworthy differences:
  ...
  * In Julia, not all data structures support logical indexing. Furthermore, logical indexing in Julia
    is supported only with vectors of length equal to the object being indexed. For example:
      ...
      * In Julia, `[1, 2, 3, 4][[true, false]]` throws a [`BoundsError`](@code-self-ref).
  ...
  * Julia's [`map`](@code-self-ref) takes the function first, then its arguments, unlike `lapply(<structure>, function, ...)`
    in R. Similarly Julia's equivalent of `apply(X, MARGIN, FUN, ...)` in R is [`mapslices`](@code-self-ref)
  ...
  * Julia's [`*`](@code-self-ref) operator can perform matrix multiplication, unlike in R. If `A` and `B` are
  ...
  * Julia's [`diag`](@code-self-ref) and [`diagm`](@code-self-ref) are not like R's.
  ...
  * In Julia, vectors and matrices are concatenated using [`hcat`](@code-self-ref), [`vcat`](@code-self-ref) and
    [`hvcat`](@code-self-ref), not `c`, `rbind` and `cbind` like in R.
  ...
  * Julia's [`max`](@code-self-ref) and [`min`](@code-self-ref) are the equivalent of `pmax` and `pmin` respectively
    in R, but both arguments need to have the same dimensions.  While [`maximum`](@code-self-ref) and [`minimum`](@code-self-ref)
    replace `max` and `min` in R, there are important differences.
  * Julia's [`sum`](@code-self-ref), [`prod`](@code-self-ref), [`maximum`](@code-self-ref), and [`minimum`](@code-self-ref) are different
  ...
  * Julia has several functions that can mutate their arguments. For example, it has both [`sort`](@code-self-ref)
    and [`sort!`](@code-self-ref).
  ...
  * Julia does not support the `NULL` type. The closest equivalent is [`nothing`](@code-self-ref), but it
    behaves like a scalar value rather than like a list. Use `x === nothing` instead of `is.null(x)`.
  * In Julia, missing values are represented by the [`missing`](@code-self-ref) object rather than by `NA`.
    Use [`ismissing(x)`](@code-self-ref) (or `ismissing.(x)` for element-wise operation on vectors) instead of
    `is.na(x)`. The [`skipmissing`](@code-self-ref) function is generally
  ...
  * In R, an idiomatic way to remove unwanted values is to use logical indexing, like in the expression
    ...
    the higher order functions [`filter`](@code-self-ref) and [`filter!`](@code-self-ref), allowing users to write
...
## Noteworthy differences from Python
  ...
  * In Julia, the commonly used `Int` type corresponds to the machine integer type (`Int32` or `Int64`), unlike in Python, where `int` is an arbitrary length integer.
    ...
    such as `Int128`, [`BigInt`](@code-self-ref) or a floating point type like `Float64`.
...
## Noteworthy differences from C/C++
  ...
  * In Julia, literal numbers without a decimal point (such as `42`) create signed integers, of type
    ...
    leading 0s. For example, `0x0` and `0x00` have type [`UInt8`](@code-self-ref), `0x000` and `0x0000` have type
    [`UInt16`](@code-self-ref), then literals with 5 to 8 hex digits have type `UInt32`, 9 to 16 hex digits type
    ...
    and 32 bit [`Float32`](@code-self-ref) bit literals are expressed as `1.0` and `1.0f0` respectively. Floating point
  ...
  * In Julia, the division operator [`/`](@code-self-ref) returns a floating point number when both operands
    are of integer type.  To perform integer division, use [`div`](@code-self-ref) or [`÷`](@ref div).
  ...
  * `//` indicates a [`Rational`](@code-self-ref) number, and not a single-line comment (which is `#` in Julia)
  ...
  * Julia does not require the use of semicolons to end statements. The results of expressions are
    ...
    do not need to end with semicolons. [`println`](@code-self-ref) or [`@printf`](@code-self-ref) can be used to
  ...
  * In Julia, the operator [`⊻`](@ref xor) ([`xor`](@code-self-ref)) performs the bitwise XOR operation, i.e.
    [`^`](@code-self-ref) in C/C++.  Also, the bitwise operators do not have the same precedence as C/C++, so
    parenthesis may be required.
  * Julia's [`^`](@code-self-ref) is exponentiation (pow), not bitwise XOR as in C/C++ (use [`⊻`](@ref xor), or
    [`xor`](@code-self-ref), in Julia)
...
- Modules (namespaces) can be hierarchical. [`import`](@code-self-ref) and [`using`](@code-self-ref) have a dual role: they load the code and make it available in the namespace. `import` for only the module name is possible (roughly equivalent to `ASDF:LOAD-OP`). Slot names don't need to be exported separately. Global variables can't be assigned to from outside the module (except with `eval(mod, :(var = val))` as an escape hatch).

- Macros start with `@`, and are not as seamlessly integrated into the language as Common Lisp; consequently, macro usage is not as widespread as in the latter. A form of hygiene for [macros](@ref man-meta) is supported by the language. Because of the different surface syntax, there is no equivalent to `COMMON-LISP:&BODY`.

- *All* functions are generic and use multiple dispatch. Argument lists don't have to follow the same template, which leads to a powerful idiom (see [`do`](@code-self-ref)). Optional and keyword arguments are handled differently. Method ambiguities are not resolved like in the Common Lisp Object System, necessitating the definition of a more specific method for the intersection.
