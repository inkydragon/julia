...
detail in [Methods](@ref man-methods), but is rooted in the type system presented here.
...
Recall that in [Integers and Floating-Point Numbers](@ref man-int-float), we introduced a variety of concrete
...
specific method named `myplus` that matches the given arguments. (See [Methods](@ref man-methods) for more
...
arguments are considered when selecting a method, rather than just the first one (see [Methods](@ref man-methods)
...
depends on both [Parametric Types](@ref man-types-parametric) and on [Methods](@ref man-methods), and is sufficiently important
...
More examples will be discussed later in [Methods](@ref man-methods).
...
next section, [Methods](@ref man-methods).
...
used to represent the arguments accepted by varargs methods (see [Varargs Functions](@ref man-func-varargs)).
...
see [Networking and Streams](@ref man-net-streams)):
...
(see [Operator Precedence and Associativity](@ref man-math-ops-precedence)), this
