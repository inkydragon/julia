# [Networking and Streams](@id man-net-streams)
...
All Julia streams expose at least a [`read`](@code-self-ref) and a [`write`](@code-self-ref) method, taking the
...
Note that [`write`](@code-self-ref) returns 11, the number of bytes (in `"Hello World"`) written to [`stdout`](@code-self-ref),
...
example, [`write`](@code-self-ref) takes the data to write as its second argument, while [`read`](@code-self-ref)
...
To read every line from [`stdin`](@code-self-ref) you can use [`eachline`](@code-self-ref):
...
or [`read`](@code-self-ref) if you wanted to read by character instead:
...
Note that the [`write`](@code-self-ref) method mentioned above operates on binary streams. In particular,
...
Note that `a` is written to [`stdout`](@code-self-ref) by the [`write`](@code-self-ref) function and that the returned
value is `1` (since `0x61` is one byte).

For text I/O, use the [`print`](@code-self-ref) or [`show`](@code-self-ref) methods, depending on your needs (see
...
The [`IOContext`](@code-self-ref) object provides this framework for associating arbitrary metadata with an IO object.
For example, `:compact => true` adds a hinting parameter to the IO object that the invoked show method
should print a shorter output (if applicable). See the [`IOContext`](@code-self-ref) documentation for a list
of common properties.

## Working with Files

Like many other environments, Julia has an [`open`](@code-self-ref) function, which takes a filename and
returns an [`IOStream`](@code-self-ref) object that you can use to read and write things from the file. For example,
...
To make this easier, there exists another invocation of [`open`](@code-self-ref) which takes a function
...
usage is somewhat simpler than the raw Unix socket API. The first call to [`listen`](@code-self-ref) will
...
UNIX domain sockets is subtle and has to do with the [`accept`](@code-self-ref) and [`connect`](@code-self-ref)
methods. The [`accept`](@code-self-ref) method retrieves a connection to the client that is connecting on
the server we just created, while the [`connect`](@code-self-ref) function connects to a server using the
specified method. The [`connect`](@code-self-ref) function takes the same arguments as [`listen`](@code-self-ref),
so, assuming the environment (i.e. host, cwd, etc.) is the same you should be able to pass the same
arguments to [`connect`](@code-self-ref) as you did to listen to establish the connection. So let's try that
...
scenes. When we called [`connect`](@code-self-ref), we connect to the server we had just created. Meanwhile,
...
the server gets to run. When we called [`connect`](@code-self-ref) the current task waited for the connection
...
As with other streams, use [`close`](@code-self-ref) to disconnect the socket:
...
One of the [`connect`](@code-self-ref) methods that does not follow the [`listen`](@code-self-ref) methods is
...
At the base of this functionality is [`getaddrinfo`](@code-self-ref), which will do the appropriate address
resolution:
...
All I/O operations exposed by [`Base.read`](@code-self-ref) and [`Base.write`](@code-self-ref) can be performed
asynchronously through the use of [coroutines](@ref man-tasks). You can create a new coroutine to
read from or write to a stream using the [`@async`](@code-self-ref) macro:
...
concurrently and wait until they've all completed. You can use the [`@sync`](@code-self-ref) macro to cause
