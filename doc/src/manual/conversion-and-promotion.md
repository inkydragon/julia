...
been mentioned in various other sections, including [Integers and Floating-Point Numbers](@ref man-int-float),
[Mathematical Operations and Elementary Functions](@ref man-math-ops), [Types](@ref man-types), and [Methods](@ref man-methods).
...
This is done via the [`convert`](@code-self-ref) function.
...
Conversion isn't always possible, in which case a [`MethodError`](@code-self-ref) is thrown indicating that `convert`
...
of numbers, and only a very limited subset of them are. Therefore in Julia the dedicated [`parse`](@code-self-ref)
...
The following language constructs call `convert`:
  ...
  * Constructing an object with [`new`](@code-self-ref) converts to the object's declared field types.
  * Assigning to a variable with a declared type (e.g. `local x::T`) converts to that type.
  * A function with a declared return type converts its return value to that type.
  * Passing a value to [`ccall`](@code-self-ref) converts it to the corresponding argument type.
...
Similar definitions exist for `AbstractString`, [`AbstractArray`](@code-self-ref), and [`AbstractDict`](@code-self-ref).
...
For instance, although every [`Int32`](@code-self-ref) value can also be represented as a [`Float64`](@code-self-ref) value,
`Int32` is not a subtype of `Float64`.

Promotion to a common "greater" type is performed in Julia by the [`promote`](@code-self-ref) function, which takes
...
behavior of `promote` is defined in terms of an auxiliary function called [`promote_rule`](@code-self-ref), which
...
In the latter case, the result type is [`BigInt`](@code-self-ref) since `BigInt` is the only type
...
The `promote_rule` function is used as a building block to define a second function called [`promote_type`](@code-self-ref),
