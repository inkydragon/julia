...
when we entered `@profile myfunc()`. The next line reflects actions taken in the [`@profile`](@code-self-ref)
macro.
...
Results from [`@profile`](@code-self-ref) accumulate in a buffer; if you run multiple pieces of code under
[`@profile`](@code-self-ref), then [`Profile.print()`](@code-self-ref) will show you the combined results. This can
be very useful, but sometimes you want to start fresh; you can do so with [`Profile.clear()`](@code-self-ref).

## Options for controlling the display of profile results

[`Profile.print`](@code-self-ref) has more options than we've described so far. Let's see the full declaration:
...
Let's first discuss the two positional arguments, and later the keyword arguments:
  ...
  * `io` -- Allows you to save the results to a buffer, e.g. a file, but the default is to print to `stdout`
    (the console).
  * `data` -- Contains the data you want to analyze; by default that is obtained from [`Profile.fetch()`](@code-self-ref),
...
to save to a file using a wide `displaysize` in an [`IOContext`](@code-self-ref):
...
[`@profile`](@code-self-ref) just accumulates backtraces, and the analysis happens when you call [`Profile.print()`](@code-self-ref).
...
## [Memory allocation analysis](@id man-profile-mem)

One of the most common techniques to improve performance is to reduce memory allocation. The
total amount of allocation can be measured with [`@time`](@code-self-ref) and [`@allocated`](@code-self-ref), and
...
you want to analyze, then call [`Profile.clear_malloc_data()`](@code-self-ref) to reset all allocation counters.
