...
println("""
This is a test file to produce/load object file from staticjit.
We will compile Tokenize in this simple demo.
You should firstly install Tokenize 0.5.21 in non-imagecodegen mode.
Usage:
...
#println("Installing package Tokenize...")
#Pkg.activate("testobj")
#Pkg.add(name = "Tokenize",version = "0.5.21")
#Pkg.activate()

@assert length(Base.ARGS) == 1
@assert Base.ARGS[1] in ["0","1"]
path = "./demo/objs"
chars = Vector{UInt8}(path)
charptr = Base.unsafe_convert(Ptr{Char}, chars)
ccall(:jl_setoutput_dir,Cvoid,(Any,),path)


if Base.ARGS[1] == 0
    all_objs = filter(x->endswith(x, ".o"), first(collect(walkdir(path)))[3])
    load_object(joinpath.(path, all_objs))
end

# test function
using Tokenize
using Tokenize.Lexers
# the following code is copied from Tokenize's test file, with MIT license.
# unsupported tests are dropped
